...
│   ├── skill-up-observer/   #   Codex plugin with a tracked install-ready Skill copy
...
- **Maintenance advice**: Validate each manifest and bundled Skill with the repository's documented plugin and Skill validators. Keep hook trust, compatibility, and local data behavior explicit in the plugin README. Plugin-local `skill-upper` directories are synchronized package artifacts, not additional authoring sources or symlinks.
...
- **Maintenance advice**: Keep its subprocess boundary argv-based, confine selected files to the caller's DSH workspace, and run `npm test` from the plugin directory after changing the plugin or `skills/skill-upper/`. Its ignored `dist/skill-upper/` copy is generated by the shared repository sync script during tests and packing.
