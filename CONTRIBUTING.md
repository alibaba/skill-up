...
│   └── skill-upper/         #   Canonical source for the eval workflow Skill
...
│   ├── codex-skill-up/      #   Codex adapter; release adds shared Skill/schema
│   └── dsh-skill-up/        #   DSH bundle; its Skill copy is generated into dist/
...
- **Maintenance advice**: This directory is the only editable source for `skill-upper`. When CLI flags, schema fields, judge types, or report formats change, update the corresponding `references/` docs and the workflow steps in `SKILL.md` in the same commit. Keep templates in `assets/` consistent with the latest `v1alpha1` schema, then run `make bundle-plugins` to prepare plugin build artifacts.
...
- **Maintenance advice**: Keep its subprocess boundary argv-based, confine selected files to the caller's DSH workspace, and run `npm test` from the plugin directory after changing the plugin or `skills/skill-upper/`. Its ignored `dist/skill-upper/` copy is generated by the shared repository bundle script during tests and packing.

### `plugins/codex-skill-up/`

- **Meaning**: The Codex host adapter. It uses lifecycle hooks and a local MCP server for explicitly attributed observation capture, review, and approved candidate-case writes.
- **Maintenance advice**: Keep Codex payload handling in this adapter. Shared observation schemas and conformance fixtures belong under `schemas/skill-observation/`; the canonical Skill remains under `skills/skill-upper/`.
...
### `schemas/skill-observation/`

- **Meaning**: Owns the host-neutral persisted observation contract and cross-host conformance fixtures.
- **Maintenance advice**: Host adapters may implement the contract in different languages, but each must validate the shared fixtures. Packaging copies the schema into host bundles; never edit a bundled copy.
