...
- 可选的事件属性是有大小限制的字符串对，不得承载核心事件语义或敏感
  评测内容。
- 事件日志启动成功后，可控收尾会恰好尝试一次发布结束生命周期的
  `run_finished` 事件，包括存在失败 Case 的已完成运行，以及以调用级错误或取
  消结束的运行。
- 最终事件发布成功的事件流恰好包含一个 `last_event: true`；该标记属于信封，不
  与某种事件类型永久绑定。
...
- JSON 字符串转义解码后，每个对象的
  [JSON 成员名](https://www.rfc-editor.org/rfc/rfc8259.html#section-4)必须唯
  一。任意层级存在重复成员名的完整换行记录都属于格式错误，并会损坏事件流；
  消费方必须先拒绝该记录，再解释其信封或 Payload。
...
件独占唯一最终标记；它不会关闭传输。Publisher 构造成功后即拥有其 Sink，调用
方不得绕过 Publisher 直接向该 Sink 发布或关闭它。`Publisher.Close` 可重复调
用，且底层 Sink 最多只关闭一次。Publisher 构造成功后的每条可控路径都必须调
用该方法，包括已经发生发布失败的路径。它会把关闭错误与粘性发布错误一起记录
并返回聚合结果。`EventSink.Close` 只报告关闭时发生的错误，不重复返回
`Publish` 已经报告的错误。`Err` 返回当前累计错误且不清除状态；关闭后，其结
果与 `Close` 的返回值一致。因此命令收尾时只需聚合一次 `Publisher.Close` 的结
果，而不再分别合并 Publisher 错误和 Sink 关闭错误。
...
- 拒绝序列化后超过 1 MiB 的事件，并返回错误，由 Publisher 将其记录为粘性
  Sink 失败；
- 只保证每个成功写入且以换行结束的记录都是合法 JSON，并且每个对象的成员名
  唯一。
...
- 相对路径以进程工作目录为基准解析；
- 父目录必须已存在；在 POSIX 系统上，命令按照
  [`os.OpenFile`](https://pkg.go.dev/os#OpenFile) 语义，以受进程 umask 影响的
  `0600` 模式创建新文件；在 Windows 上，新文件使用
  [从父目录继承的默认安全描述符](https://learn.microsoft.com/en-us/windows/win32/fileio/file-security-and-access-rights)，
  v1 不承诺提供等同 `0600` 的 ACL；截断已有文件不会改变其模式或 ACL，v1 也不
  修改 ACL；
...
功打开文件，并且成功发布 `run_started` 和初始 `preparing` `run_progress` 快照
后，事件日志才算启动完成。打开失败时不产生事件流。如果任一启动事件发布失
败，命令会关闭 Publisher，并在加载凭据或调用 Agent 前退出；此时不尝试发布
`run_finished`。文件属于不完整事件流：它可能为空、以部分写入结尾，或者只包
含一条完整的 `run_started` 前缀。
...
3. 校验并打开请求的 JSONL Sink，创建 Publisher 和生命周期 Emitter，发布
   `run_started` 和初始 `preparing` 进度快照。任一事件发布失败时采用上述启
   动行为。
...
6. 停止心跳，进入 `finalizing`，清除因 invocation 取消/错误而已不再活动的任
   ...
   调用级失败选择 `ERROR`。
7. 通过 `PublishLast` 发布 `run_finished`，调用 `Publisher.Close`，并使用
   `errors.Join` 合并评测错误、Case 结果错误和 `Close` 返回的 Publisher 聚合
   错误。
...
- 无法创建或打开显式请求的事件日志，或者任一启动事件发布失败时，启动失败。
  启动事件发布失败后会关闭 Publisher，不开始评测，也不尝试发布最终事件。
...
- 只有事件日志启动成功后，Sink 再发生失败时评测才可以继续生成报告。所有粘
  性 Sink 错误最终汇总到命令错误中，并使命令在收尾后以非零状态退出。
...
透明中继保留、并发 Publisher 排序、粘性 Sink 失败、Publisher 重复关闭、发布失
败后仍关闭底层 Sink、发布与关闭错误聚合、信封和嵌套对象中的重复成员名（包括
转义后等价的名称）、中间格式错误记录、尾部残缺记录、UTF-8 和记录大小限制、
Benchmark、多 iteration、假时钟心跳和端到端消费测试。Planner 和生命周期
Emitter 测试必须验证不透明任务身份与包含路径分隔符、百分号和非 ASCII 文本的
源 Case ID 解耦。命令测试还必须覆盖参数帮助、可重复属性、空路径和 `-`、
dry-run 冲突、相对路径解析、POSIX `0600` 创建模式、跨平台创建/截断行为、已有
文件权限保留、父目录不存在、iteration 工作区冲突、第一条和第二条启动事件分别
发布失败、每种启动失败下底层 Sink 只关闭一次且不尝试发布最终事件、终止关闭失
败，以及未指定参数时行为不变。生命周期测试必须覆盖事件日志成功启动后的凭据
失败、Case 失败但状态为 `COMPLETED`、取消后没有剩余活动任务、invocation
Context 取消后的终止事件发布、部分完成的 `ERROR`、并发 Case 下的进度，以及
恰好一个通用 `last_event` 标记。
