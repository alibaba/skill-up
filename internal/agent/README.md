...
- **Artifacts**: `GeneratedFiles` always includes the workspace `stdout.json`; when a session is found, the **absolute path** of that file is appended (without copying it into the workspace), so that the eval side can `DownloadFile` it for archival.
