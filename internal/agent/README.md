...
- **Editions and session-to-workspace mapping**: `engine.kwargs.edition` selects Global (default) or CN. Global uses `qodercli`, `QODER_PERSONAL_ACCESS_TOKEN`, and `$HOME/.qoder/projects`; CN uses `qodercn`, `QODERCN_PERSONAL_ACCESS_TOKEN`, and `$HOME/.qoder-cn/projects`. Both editions install project Skills under `.qoder/skills`. Session lookup compares the runtime workspace with candidates under the selected root instead of assuming a directory-key encoding.
...
- **Concurrent processes**: QoderCLI 1.1.21 has raced while extracting the fixed `/tmp/qodercli-natives-<version>-<platform>` directory; a root-owned remnant in an image can also cause `EPERM`. Three runs with QoderCLI 1.1.41 in the Aone component image completed two simultaneous cases on September 24, 2026. This is evidence for that image and version, not a general QoderCLI concurrency limit. Verify the case results and event log in each image before raising `cases.parallelism`; keep serial execution on an image that still shows native-extraction errors. `skill-up` does not add an agent-layer lock, so configured case parallelism remains effective.
